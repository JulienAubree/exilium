...
export const planets = pgTable(
  'planets',
  {
    ...
    planetClassId: varchar('planet_class_id', { length: 64 }).references(() => planetTypes.id, {
      onDelete: 'set null',
    }),
    ...
    resourcesUpdatedAt: timestamp('resources_updated_at', { withTimezone: true })
      .notNull()
      .defaultNow(),
  ...
  (table) => [
    uniqueIndex('unique_coordinates').on(
      table.galaxy,
      table.system,
      table.position,
      table.planetType,
    ...
    index('planets_user_status_idx').on(table.userId, table.status),
  ],
);
