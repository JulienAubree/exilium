...
export const fleetPhaseEnum = pgEnum('fleet_phase', [
  'outbound',
  'prospecting',
  'mining',
  'exploring',
...
export const fleetEvents = pgTable(
  ...
  (table) => [
    index('fleet_events_arrival_idx')
      .on(table.arrivalTime)
      .where(sql`status = 'active'`),
    index('fleet_events_user_idx').on(table.userId),
    index('fleet_events_origin_planet_idx').on(table.originPlanetId),
    index('fleet_events_target_planet_idx').on(table.targetPlanetId),
    index('fleet_events_user_status_idx').on(table.userId, table.status),
  ],
);
