 ...
 */
/**
 * Contexte par-tir threadé jusqu'à `fireShot`. Il ne portait à l'origine que
 * des hooks de boss V9 (système retiré) ; il reste le canal de la seule stat
 * qui en dépendait encore.
 */
interface ShotCtx {
  /**
   ...
   * fork Armement). Issue des multiplicateurs ATTAQUANT. 0 = inactif.
   */
...
function fireShot(
  ...
  const shieldPierce = (shotCtx && attackerSide === 'attacker') ? shotCtx.attackerShieldPierce : 0;
...
export function simulateCombat(input: CombatInput): CombatResult {
  ...
  // effectif négatif. Threadé via shotCtx (gaté attackerSide==='attacker').
  const attackerShieldPierce = Math.min(0.999, Math.max(0, attackerMultipliers.shieldPierce ?? 0));

  const shotCtx: ShotCtx = { attackerShieldPierce };
