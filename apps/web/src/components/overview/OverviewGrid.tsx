...
function GridHeader({ icon: Icon, label, color, count }: { icon: any; label: string; color: string; count?: number }) {
  return (
    <div className="flex items-center justify-between mb-2">
      <h3 className={`text-xs font-semibold flex items-center gap-1.5 ${color}`}>
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      ...
      <GridCard onClick={() => navigate('/production?tab=defenses')} className="col-span-2 lg:col-span-1">
        ...
        {shieldLevel > 0 && (
          <div className="flex items-center gap-1.5 px-1.5 py-1 rounded bg-cyan-500/[0.06] border border-cyan-500/10 mb-1.5 text-xs">
        ...
        {defenses.length > 0 ? (
          ...
          <p className="text-xs text-muted-foreground italic">Aucune défense</p>
        )}
      </GridCard>
      ...
      <GridCard onClick={() => navigate('/fleet/flagship')} className="col-span-2 lg:col-span-1">
        ...
        {flagship ? (
          flagship.planetId === currentPlanetId ? (
            ...
            <p className="text-xs text-muted-foreground italic">Pas sur cette planète</p>
          )
        ) : (
          <p className="text-xs text-muted-foreground italic">Aucun vaisseau amiral</p>
        )}
      </GridCard>
    </div>
  );
