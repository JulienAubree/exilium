...
export function FlagshipNamingModal({ open, onClose }: FlagshipNamingModalProps) {
  ...
  const isValid =
    name.length >= 2 && name.length <= 32 && NAME_REGEX.test(name) && selectedHull !== null;
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        Nos ingenieurs ont remis ce vaisseau en etat. Choisissez sa coque et donnez-lui un nom,
        Commandant.
      </p>
              ...
              >
                <div
                  className={cn(
                    ...
                    isSelected ? styles.accent : 'text-foreground',
                  )}
                >
                  {hull.name}
                </div>
      ...
      <div className="mt-4 space-y-3">
        ...
        <div>
          <label className="text-xs font-medium text-foreground">Description (optionnelle)</label>
          <textarea
            value={description}
      ...
      <button
        onClick={() =>
          createMutation.mutate({
            ...
            hullId: selectedHull as 'combat' | 'industrial' | 'scientific',
          })
        }
