...
export function AnomalyEngageModal({ open, onClose }: Props) {
  ...
  const weaponProfiles = useMemo<WeaponProfilePreview[]>(() => {
    ...
    const hullShotCount = Number(
      (flagship as { effectiveStats?: { shotCount?: number } }).effectiveStats?.shotCount ??
        (flagship as { shotCount?: number }).shotCount ??
        1,
    );
    out.push({
      source: 'hull',
    ...
    const moduleLoadout = ((flagship as { moduleLoadout?: unknown }).moduleLoadout ?? {}) as Record<
      string,
      | { weaponEpic?: string | null; weaponRare?: string | null; weaponCommon?: string | null }
      | undefined
    >;
    ...
    for (const m of (allModules ?? []) as Array<{
      id: string;
      name: string;
      effect?: unknown;
      kind?: string;
  ...
  return (
    <Modal
      ...
      className="max-w-md lg:max-w-md rounded-lg border-violet-500/30 shadow-2xl shadow-violet-950/40 p-0"
    >
        ...
        <div className="relative flex items-center gap-3 px-5 py-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-violet-200/70 mt-0.5 truncate">
              ...
              <span className="text-violet-200/60"> · {hullName}</span> part seul — modules +
              charges feront la différence.
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <ul className="space-y-0.5 text-[11px]">
            {weaponProfiles.length === 0 ? (
              <li className="text-muted-foreground italic">
                Aucun profil — vérifier la configuration du vaisseau amiral.
              </li>
            ) : (
          ...
          <div className="grid grid-cols-2 gap-2 text-[10px]">
            <div className="flex items-center justify-between rounded bg-card/40 px-2 py-1">
              ...
              <span className="font-mono tabular-nums text-foreground/90">
                ~{enemyFpAtDepth1.toLocaleString()}
              </span>
            </div>
            <div className="flex items-center justify-between rounded bg-card/40 px-2 py-1">
