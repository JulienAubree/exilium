...
const BUFF_TONES: Record<BossBuff, string> = {
  ...
  hull_repair:
    'border-emerald-500/40 hover:bg-emerald-950/40 hover:border-emerald-400/70 text-emerald-100',
  ...
  extra_charge:
    'border-violet-500/40 hover:bg-violet-950/40 hover:border-violet-400/70 text-violet-100',
  module_unlock:
    'border-orange-500/40 hover:bg-orange-950/40 hover:border-orange-400/70 text-orange-100',
};
...
export function AnomalyBossVictoryModal({ open, boss, buffChoices, onClose }: Props) {
  ...
  const applyMutation = trpc.anomaly.applyBossBuff.useMutation({
    onSuccess: (data) => {
      ...
      addToast(`Récompense activée : ${BUFF_LABELS[data.appliedBuff.type]}`, 'success');
      onClose();
    },
    onError: (err) => {
  ...
  return (
    <Modal
      ...
      className="max-w-lg lg:max-w-lg rounded-xl border-amber-500/40 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 shadow-[0_0_40px_rgba(251,191,36,0.18)] p-0"
    >
      {/* Header victoire */}
          ...
          {buffChoices.map((c) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-bold tracking-tight">{BUFF_LABELS[c.type]}</div>
