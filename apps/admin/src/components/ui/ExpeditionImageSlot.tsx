...
interface ExpeditionImageSlotProps {
  /** Slot key — alphanum + `-` / `_` only (e.g. `sector-theta-7`, `event-epave-recyclable`). */
  slot: string;
  /** Current public path (typically `/assets/expedition/<slot>.webp`). */
  value: string;
...
/**
 * Upload widget pour un slot d'image d'expédition (secteur ou événement).
 * Calqué sur AnomalyImageSlot — même backend `/admin/upload-asset` avec
 * `category=expedition`.
 */
export function ExpeditionImageSlot({
  slot,
  value,
  aspect = '16/9',
  label,
  ...
  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const path = `/assets/expedition/${slot}.webp`;
      onChange(path);
