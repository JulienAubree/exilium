...
This requires importing `computeFleetFP`, `scaleFleetToFP`, `computeUnitFP` and types from `@exilium/game-engine`.
...
To compute FP in the admin, import `computeFleetFP` and `computeUnitFP` from `@exilium/game-engine` and use the ship stats from `useGameConfig()`:

```typescript
import { computeFleetFP, computeUnitFP } from '@exilium/game-engine';
...
Run: `cd /Users/julienaubree/_projet/exilium && npx turbo build`
