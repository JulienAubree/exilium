...
## ⚠️ `pnpm typecheck` N'EST PAS une opération en lecture seule
`typecheck` passe par turbo, qui déclenche `@exilium/api:build` (`tsc`) — donc **il réécrit `apps/api/dist` et `packages/*/dist`**. Or `/opt/exilium` **est** le checkout de production : les processus PM2 tournent depuis ces `dist`. Un simple `typecheck` sur une branche de travail suffit à mettre du code non déployé à un redémarrage près de la prod (les processus en cours gardent leur copie en mémoire, un respawn prend la nouvelle).
- `apps/web/dist` (servi en direct par Caddy) n'est PAS concerné : `@exilium/web:typecheck` est un `tsc --noEmit`.
- Après avoir travaillé sur une branche : `git checkout main && pnpm --filter @exilium/api build && pnpm --filter @exilium/db build` pour remettre les `dist` à l'état déployé. Vérifier avec un `grep` d'un symbole de la branche dans `apps/api/dist`.
- Attention aussi : le simulateur (`packages/game-sim`) lit le **`dist` compilé** de `@exilium/db`, jamais la source. Toute mesure de rééquilibrage sans `pnpm --filter @exilium/db build` préalable mesure l'ancienne configuration et conclut « aucun effet ».

## Carte neuve — les clés qui font un monde
- **`world_seed`** : la génération procédurale est seedée par `galaxy:system:position` UNIQUEMENT. À `0`, deux univers de mêmes dimensions sont **identiques case par case** — une « carte neuve » rendrait l'ancienne. Toute valeur non nulle donne un vrai monde neuf. Branchée aux 4 points de génération (vue galaxie, exploration, colonisation, rapport d'exploration) via `worldSeed(config)`.
- **`topology`** : `ring` (la carte se referme) ou `bounded` (la carte a des bords, ce que veut la fiction). `buildFleetConfig` transmet enfin `maxGalaxies`/`maxSystems` : sans eux le moteur retombait sur 9/499 en dur et le repli de l'anneau se calculait sur des systèmes inexistants.
- **`economy_speed`** : multiplicateur plat de production, posé dans `calculateProductionRates` donc valable pour les 4 chemins d'un coup. **Seul levier de rythme homothétique** — raidir un exposant est non-monotone et peut *ralentir* le jeu.
- **`belt_profiles`** : présence, poids et quantités par position de ceinture. Étaient en dur avec une position typée `8 | 16`.
- ⚠️ Changer les **dimensions** exige de le faire en base ET dans `packages/db/src/game-config-data.ts`, sinon une base recréée de zéro repart sur l'ancienne taille.
- **Le reset** : `packages/db/src/scripts/reset-univers.sql`, joué à la main par Julien, jamais dans `drizzle/`. Backup + `pm2 stop` avant, contrôle final transactionnel qui annule si `world_seed` vaut 0.
